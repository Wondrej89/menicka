...
const FALLBACK_MENU_BY_RESTAURANT = {
  'bife-restaurant': [
    ...
    { type: 'main', name: 'Caesar salát s kuřecím masem', price: 175 }
  ],
  'corleone-andel': [
    { type: 'soup', name: 'Rajčatová polévka s bazalkou', price: 45 },
    { type: 'main', name: 'Spaghetti Bolognese', price: 185 },
    { type: 'main', name: 'Kuřecí prsa se špenátem a rizotem', price: 189 },
  ...
  smichovna: [
    ...
    { type: 'main', name: 'Grilovaný hermelín, šťouchané brambory', price: 179 }
  ],
  'smichovska-formanka': [
    ...
    { type: 'main', name: 'Smažený květák, vařené brambory', price: 165 }
  ],
  'u-mamlasu': [
    { type: 'soup', name: 'Česnečka se sýrem', price: 49 },
    { type: 'main', name: 'Vídeňský řízek z vepřové kýty, bramborová kaše', price: 189 },
    { type: 'main', name: 'Hovězí na houbách, jasmínová rýže', price: 185 },
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/restaurants') {
    return sendJson(res, RESTAURANTS.map(({ id, name, url: menuUrl }) => ({ id, name, url: menuUrl })));
  }
  ...
  if (url.pathname === '/api/menus') {
    ...
    return sendJson(res, { generatedAt: new Date().toISOString(), weekday: today, menus });
  }
...
server.listen(PORT, () => {
  console.log(`Meníčka běží na http://localhost:${PORT}`);
});
...
function parseWeeklyPage(html, { today }) {
  ...
  return {
    ...
    message: items.length ? 'Načteno.' : 'Nepodařilo se rozpoznat položky menu.',
    items
  };
}
